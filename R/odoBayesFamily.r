...
odoMultiSpecies_AllGroups <- function(species_index_df, nk = 10, ncores = 7) {
  ...
  # --- Estrazione famiglia per ogni specie da dfo ---
  family_lookup <- dfo %>%
  ...
  cat("Distribuzione specie per famiglia:\n")
  print(table(family_vec))
  ...
  # 5. CALCOLARE group_mean_psi POST-HOC (per gruppo gindex)
  cat("\nCalcolando statistiche di gruppo (gindex)...\n")
  ...
  # 7b. CALCOLO STIMA PER FAMIGLIA (post-hoc)
  cat("\nCalcolando stima per famiglia...\n")

  family_results <- list()
  for (fam in all_families) {
    species_in_family <- which(family_vec == fam)

    if (length(species_in_family) == 0) {
      cat("Nessuna specie nella famiglia", fam, "\n")
      next
    }

    family_mean_by_year <- matrix(
      NA,
      nrow = nrow(mean_psi_samples),
      ncol = length(anni_num)
    )
    ...
    for (t in 1:length(anni_num)) {
      ...
      if (length(cols_family_year) > 0) {
        family_mean_by_year[, t] <- rowMeans(
          mean_psi_samples[, cols_family_year, drop = FALSE]
        )
      }
    }
    ...
    family_results[[fam]] <- list(
      family = fam,
      n_species = length(species_in_family),
      plot_data = family_plot_data,
      samples = family_mean_by_year
    )
  }

  # 8. PLOT: confronto tra gruppi gindex
  ...
  pGroups <- all_group_data %>%
    ...
    labs(
      title = "Occupancy across HV Groups",
      x = "Year",
      y = "Mean Occupancy"
    ) +
  ...
  ggsave(
    paste0(
      "output/output_bayes/multispecies/MULTISPECIES_ALL_GROUPS_HV",
      today(),
      ".pdf"
    ),
    width = 12,
  ...
  # 9b. PLOT: confronto tra famiglie
  ...
  pFamilies <- all_family_data %>%
    ...
    scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
    labs(
      title = "Occupancy across Families",
      x = "Year",
      y = "Mean Occupancy"
    ) +
    theme_bw(base_size = 14) +
  ...
  # 11. CONFRONTI STATISTICI: tra gruppi gindex
  ...
  #     Diff_lower = apply(diff_samples, 2, quantile, probs = 0.025, na.rm = TRUE),
  #     Diff_upper = apply(diff_samples, 2, quantile, probs = 0.975, na.rm = TRUE)
  #   )
  #   cat("\n--- Gruppo", g, "(n =", group_results[[g]]$n_species, "specie) vs Globale ---\n")
  #   cat("  Prob(Gruppo > Globale):", round(prob_above, 3), "\n")
  #   cat("  Differenza media (pooled):", round(mean(diff_samples, na.rm = TRUE), 3), "\n")
  ...
  # pDiff <- diff_all %>%
  #   filter(Anno > 2004) %>%
  #   ggplot(aes(x = Anno, y = Diff_mean, color = factor(Group), fill = factor(Group))) +
  #   geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  #   geom_ribbon(aes(ymin = Diff_lower, ymax = Diff_upper), alpha = 0.2, color = NA) +
  #   geom_line(linewidth = 1.2) +
  ...
  saveRDS(
    list(
      ...
      # global_results = global_results,   # rimosso: stima globale non più calcolata
      species_results = species_results,
      comparisons = comparisons,
      # group_vs_global = group_vs_global, # rimosso: confronto vs globale non più calcolato
      rhat = rhat_df
    ),
    paste0(
      "output/output_bayes/multispecies/MULTISPECIES_ALL_GROUPS_HV",
