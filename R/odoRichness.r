...
odoRichness_robust <- function(
  ...
  species_site_counts <- dfo %>%
    mutate(cellcodeX = as.character(cellcodeX)) %>%
    filter(cellcodeX %in% as.character(all_cells), species %in% all_species) %>%
    group_by(species) %>%
  ...
  for (i in seq_len(nrow(stratum_alloc))) {
    log_msg(sprintf(
      "    Strato %d (%d disponibili) → %d selezionati",
  ...
  set.seed(site_seed)
  sites_stratified <- do.call(
    c,
    lapply(stratum_alloc$stratum, function(st) {
      pool_st <- pool %>% filter(stratum == st)
      n_st <- stratum_alloc$n_alloc[stratum_alloc$stratum == st]
      if (n_st == 0 || nrow(pool_st) == 0) {
        return(character(0))
      }
      pool_st %>%
  ...
  log_msg(
    ...
    length(sites_forced),
    " forzati + ",
    length(sites_stratified),
    " stratificati)"
  )

  eff_sel <- site_meta %>%
    filter(cellcodeX %in% selected_cells) %>%
    pull(effort)
  eff_exc <- site_meta %>%
    filter(!cellcodeX %in% selected_cells) %>%
    pull(effort)
  log_msg(sprintf(
    "  Effort medio — selezionati: %.1f  |  esclusi: %.1f anni",
    mean(eff_sel),
    mean(eff_exc)
  ))
  ...
  effort_subset <- effort_matrix[
    match(as.character(all_cells), rownames(effort_matrix)),
    ,
    drop = FALSE
  ]
  ...
  code <- nimbleCode({
    omega ~ dunif(0, 1)
    mu_psi1 ~ dnorm(0, sd = 2)
    sigma_psi1 ~ dexp(1)
    mu_gamma ~ dnorm(0, sd = 2)
    sigma_gamma ~ dexp(1)
    mu_eps ~ dnorm(0, sd = 2)
    sigma_eps ~ dexp(1)
    mu_p ~ dnorm(0, sd = 2)
  ...
  make_inits <- function(seed_val) {
    ...
    list(
      ...
      sigma_psi1 = 0.5,
      mu_gamma = -1,
      sigma_gamma = 0.5, # gamma moderata ~0.27
      mu_eps = -1,
      sigma_eps = 0.5, # eps moderata ~0.27
      mu_p = 0,
      sigma_p = 0.5,
  ...
  if (!is.finite(lp_init)) {
    stop(
      "Log-prob iniziale = -Inf o NaN: valori iniziali incompatibili. ",
  ...
  richness_samples <- samples_mat[,
    grep("^richness\\[", colnames(samples_mat)),
    drop = FALSE
  ]
  ...
  if (omega_summary["upper.97.5%"] > 0.97) {
    log_msg(
      "ATTENZIONE: omega > 0.97 → n_zero troppo piccolo. ",
      "Ripeti con n_zero = ",
      n_zero + 10
    )
  }
  ...
  log_msg(sprintf(
    "N_pool: %.1f [%.1f - %.1f]  (osservate: %d | stimate non rilevate: %.1f)",
    N_pool_summary["mean"],
  ...
  pRichness <- richness_summary %>%
    ...
    geom_ribbon(
      aes(ymin = richness_lower, ymax = richness_upper),
      alpha = 0.25,
      fill = "steelblue"
    ) +
  ...
  site_meta_plot <- site_meta %>%
    mutate(
      ...
      tipo = factor(
        tipo,
        levels = c("Forzato (specie rare)", "Stratificato", "Escluso")
      )
    )
