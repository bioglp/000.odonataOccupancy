...
[vignette](https://cran.r-project.org/web/packages/unmarked/vignettes/unmarked.html)
...
occProb <- function(nomeSp) {
  ...
  # calcola una previsione di occupancy per ogni anno
  predict(fm, 'det')
}
...
```{r pcount, include=TRUE, echo=F}
#name <- "Aeshna cyanea"
odoPCount <- function(name) {
  df_sp <- dfp %>% 
    filter(species == name)
