...
dfo <- dfo %>% filter(species!='Odonata sp.')
dfo <- dfo %>% filter(species!='Lestes sp.')
```
...
plot1 <- all_species_df %>% 
  ...
  geom_point(aes(col=year_group),size = 4) +
...
plot5 <- all_species_df %>% 
  filter(species==nomeSp) %>% 
  group_by(year_group) %>% 
  summarise(mean.est=geometric_mean(estimate),
            dev.est=sd(estimate),
            lower=mean.est-dev.est,
            upper=mean.est+dev.est) %>% 
  mutate(year = if (nomeSp == "Trithemis annulata") {
    c(2019, 2025)[seq_len(n())]
  } else {
    c(1999, 2004, 2009, 2014, 2019, 2025)[seq_len(n())]
  }) %>%
