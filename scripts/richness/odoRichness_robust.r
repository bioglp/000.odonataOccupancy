...
odoRichness_robust <- function(species_index_df,
  ...
  effort_std_mat <- matrix(
    (as.numeric(effort_matrix[rownames(effort_matrix) %in% all_cells, ]) -
       mean(as.numeric(effort_matrix), na.rm = TRUE)) /
      sd(as.numeric(effort_matrix), na.rm = TRUE),
    nrow = n_sites, ncol = n_years
  )
  ...
  make_inits <- function(seed_val) {
    ...
    for (s in seq_len(n_species)) {
      ...
      p_init <- runif(1, 0.4, 0.6)
      for (i in seq_len(n_sites))
        for (t in seq_len(n_years))
          z_init[s,i,t] <- if (!is.na(y_mat[i,t])) {
            if (y_mat[i,t]==1) 1L else rbinom(1,1,0.3)
          } else rbinom(1,1,p_init)
    }
    list(
      omega=runif(1,0.3,0.7),
      w=c(rep(1L,n_species), rep(0L,n_zero)),
      mu_psi1=rnorm(1,0,0.5),  sigma_psi1=rexp(1,1),
      mu_gamma=rnorm(1,0,0.5), sigma_gamma=rexp(1,1),
      mu_eps=rnorm(1,0,0.5),   sigma_eps=rexp(1,1),
      mu_p=rnorm(1,0,0.5),     sigma_p=rexp(1,1),
      alpha_psi1=rnorm(n_total,0,0.5),  alpha_gamma=rnorm(n_total,0,0.5),
      alpha_eps=rnorm(n_total,0,0.5),   alpha_p=rnorm(n_total,0,0.5),
      beta_tin_psi1=rnorm(1,0,0.5),     beta_year_gamma=rnorm(1,0,0.5),
      beta_year_eps=rnorm(1,0,0.5),     beta_effort=rnorm(1,0,0.5),
      z=z_init
    )
  }
